...
const NavLink = ({ to, children, className }) => (
  <li>
    <Link
      to={to}
      className={`flex items-center p-3 rounded-lg transition-all duration-300 hover:bg-white/10 hover:backdrop-blur-md hover:text-primary hover:translate-x-1 ${className}`}
    >
      {children}
    </Link>
  </li>
...
export default function Layout({ children }) {
  ...
  const userUnidad = user?.es_auditor
    ? "Rol: Auditor"
    : user?.comunidad?.nombre || user?.comunidad_nombre || "Unidad Operativa";
        ...
        <div className="w-full navbar bg-base-100/90 backdrop-blur-sm sticky top-0 z-30 border-b border-white/5 px-6">
          ...
          <div className="flex-none">
            <div className="dropdown dropdown-end">
              <label
                tabIndex={0}
                className={`btn btn-ghost btn-circle avatar ring ring-primary ring-offset-base-100 ring-offset-2 ${!avatarUrl ? "placeholder" : ""}`}
              >
                <div
                  className={`bg-neutral text-neutral-content rounded-full w-10 ${avatarUrl ? "" : ""}`}
                >
                  {avatarUrl ? (
                    <img
                      src={avatarUrl}
                    ...
                    <span className="text-xl font-bold">
                      {getInitials(user?.nombre || user?.username || "Usuario")}
                    </span>
                  )}
                </div>
              </label>
              <ul
                tabIndex={0}
                className="menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow bg-base-200 rounded-sm w-52 border border-white/10"
              >
                <li>
                  <a onClick={triggerFileInput} className="hover:text-primary">
                    <UploadIcon /> Cambiar Avatar
                  </a>
                </li>
                <li>
                  <a onClick={handleLogout} className="hover:text-primary">
